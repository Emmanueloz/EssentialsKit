...
func _on_body_entered(body: Node2D) -> void:
	...
	if health_comp == null:
		print("[BuffZone2D] No HealthComponent found on: ", body.name, " - modifiers not applied")
		return
	...
	if apply_on_enter:
		...
		print("[BuffZone2D] Applied modifiers to: ", body.name)
	...
	if remove_on_exit:
		...
		if health_comp != null and body in _applied_modifiers:
			for modifier in _applied_modifiers[body]:
				...
				print("[BuffZone2D] Removed modifier '%s' from: %s" % [modifier.name, body.name])
			_applied_modifiers.erase(body)

func _apply_all_modifiers(body: Node2D, health_comp: HealthComponent) -> void:
