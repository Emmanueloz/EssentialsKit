## Area-based ongoing damage hitbox for 2D games.
## Applies damage periodically while entities remain inside the area.
class_name OngoingDamageHitbox2D
extends Area2D
...
func _on_body_entered(body: Node2D) -> void:
	...
	if health_comp == null:
		print("[OngoingDamageHitbox2D] No HealthComponent found on: ", body.name)
		return
	...
	if _targets_in_area.size() > 0 and not is_instance_valid(_timer_damage):
		...
		_timer_damage.start()
		print("[OngoingDamageHitbox2D] Damage timer started")
...
func _on_recurre_damage() -> void:
	for hit_target in _targets_in_area:
		...
		if hit_target.hurtbox:
			...
			if hurtbox_mult != 1.0:
				print("[OngoingDamageHitbox2D] Hurtbox '%s' multiplier: x%.2f" % [hit_target.hurtbox.get_zone_name(), hurtbox_mult])
		...
		print("[OngoingDamageHitbox2D] Ongoing damage to %s: %.1f" % [hit_target.node.name, final_damage])
