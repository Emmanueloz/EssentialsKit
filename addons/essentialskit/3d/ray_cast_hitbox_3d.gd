## RayCast-based damage hitbox for 3D games.
...
func _physics_process(delta: float) -> void:
	...
	if damage == null:
		print("[RayCastHitbox3D] No DamageType assigned!")
		return
...
func _apply_single_damage(target: Node3D, health_comp: HealthComponent) -> void:
	...
	if hurtbox:
		...
		if hurtbox_mult != 1.0:
			print("[RayCastHitbox3D] Hurtbox '%s' multiplier: x%.2f" % [hurtbox.get_zone_name(), hurtbox_mult])
...
func _handle_continuous_damage(target: Node3D, health_comp: HealthComponent) -> void:
...
func _apply_continuous_damage(target: Node3D, health_comp: HealthComponent) -> void:
	...
	print("[RayCastHitbox3D] Continuous damage to %s: %.1f" % [target.name, final_damage])
...
func _stop_continuous_damage(target: Node3D) -> void:
...
func _get_hurtbox_from_body(body: Node3D) -> Hurtbox3D:
	...
	for hurtbox in hurtboxes:
		...
		var shape_node = _get_child_of_type(hurtbox, CollisionShape3D)
		if shape_node == null:
			continue
...
func _get_all_hurtboxes(body: Node3D) -> Array[Hurtbox3D]:
	var result: Array[Hurtbox3D] = []
	for child in body.get_children():
		if child is Hurtbox3D and child.can_be_hit:
			result.append(child)
	return result
...
func _apply_modifiers(target: Node) -> void:
	...
	for mod in damage.modifiers_on_hit:
		...
		print("[RayCastHitbox3D] Applied modifier: %s" % mod.name)
