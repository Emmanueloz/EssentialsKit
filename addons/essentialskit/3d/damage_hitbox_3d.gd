## Area-based damage hitbox for 3D games.
...
func _on_body_entered(body: Node3D) -> void:
	...
	if health_comp == null:
		print("[DamageHitbox3D] No HealthComponent found on: ", body.name)
		return
	
	if damage == null:
		print("[DamageHitbox3D] No DamageType assigned!")
		return
	...
	if hurtbox:
		...
		if hurtbox_mult != 1.0:
			print("[DamageHitbox3D] Hurtbox '%s' multiplier: x%.2f" % [hurtbox.get_zone_name(), hurtbox_mult])
...
func _apply_modifiers(target: Node) -> void:
	...
	for mod in damage.modifiers_on_hit:
		...
		print("[DamageHitbox3D] Applied modifier: %s" % mod.name)
